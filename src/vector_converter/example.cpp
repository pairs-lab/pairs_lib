...
/**  \file
     ...
     This example may be run after building *pairs_lib* by executing `rosrun pairs_lib vector_converter_example`.
...
/**  \example "vector_converter/example.cpp"
     ...
     It may be run after building *pairs_lib* by executing `rosrun pairs_lib vector_converter_example`.
 */
...
// We need to define our own specialization of the pairs_lib::impl::convertFrom() and pairs_lib::impl::convertTo() functions for this type. convert() will then work automatically.
namespace pairs_lib
{
  namespace impl
  {

    // convertFrom specialization for MyPoint (pairs_lib::impl::unw_t is just a shorthand for std::tuple<double, double, double>)
...
#include <pairs_lib/vector_converter.h>
...
int main()
{
  ...
  // the pairs_lib::impl::convertTo() function is used as a shortcut for initialization of the object
  ...
  // around the line `#include <pairs_lib/vector_converter.h>` (see above).
  const auto from = pairs_lib::impl::convertTo<pcl::PointXYZ>(xo, yo, zo);

  // This is the main functionality - one-liner for converting between two vector types.
  const auto to = pairs_lib::convert<Eigen::Vector3d>(from);
  const auto to2 = pairs_lib::convert<geometry_msgs::Vector3>(to);
  ...
  const auto custom1 = pairs_lib::convert<MyPointOK>(from); // works
  ...
  const auto custom2 = pairs_lib::convert<MyPoint>(from); // now also works
