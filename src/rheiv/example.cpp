...
/**  \file
     ...
     This example may be run after building *pairs_lib* by executing `rosrun pairs_lib rheiv_example`.
...
pairs_lib::zs_t f_z(const pairs_lib::xs_t& xs)
{
  return xs;
}
...
int main()
{
  ...
  const pairs_lib::dzdx_t dzdx = pairs_lib::dzdx_t::Identity();

  const int n_pts = 4;
  pairs_lib::xs_t xs(3, n_pts);
  xs <<
    0, 1, 0, 10,
    0, 0, 1, 0,
  ...
  const pairs_lib::P_t P = pairs_lib::P_t::Identity();
  pairs_lib::Ps_t Ps;
  Ps.reserve(n_pts);
  ...
  pairs_lib::rheiv_t rheiv(f_z, dzdx, 1e-15, 10, std::chrono::milliseconds(1), 1);
  std::cout << "running fit" << std::endl;
  pairs_lib::theta_t theta;
  try
  {
    theta = rheiv.fit(xs, Ps);
  } catch (const pairs_lib::eigenvector_exception& ex)
  {
