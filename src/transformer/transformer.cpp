...
using test_t = pairs_msgs::ReferenceStamped;
/* using test_t = pcl::PointCloud<pcl::PointXYZ>; */
template std::optional<test_t> pairs_lib::Transformer::transform<test_t>(const test_t& what, const geometry_msgs::TransformStamped& to_frame);
template std::optional<test_t::Ptr> pairs_lib::Transformer::transform<test_t>(const test_t::Ptr& what, const geometry_msgs::TransformStamped& to_frame);
template std::optional<test_t::Ptr> pairs_lib::Transformer::transform<test_t>(const test_t::ConstPtr& what, const geometry_msgs::TransformStamped& to_frame);
template std::optional<test_t> pairs_lib::Transformer::transformSingle<test_t>(const test_t& what, const std::string& to_frame);
template std::optional<test_t::Ptr> pairs_lib::Transformer::transformSingle<test_t>(const test_t::Ptr& what, const std::string& to_frame);
template std::optional<test_t::Ptr> pairs_lib::Transformer::transformSingle<test_t>(const test_t::ConstPtr& what, const std::string& to_frame);

template std::optional<pairs_msgs::Reference> pairs_lib::Transformer::transformSingle<pairs_msgs::Reference>(const std::string& from_frame, const pairs_msgs::Reference& what, const std::string& to_frame, const ros::Time& time_stamp = ros::Time(0));
...
namespace tf2
{
  ...
  {
    const geometry_msgs::Point pt = pairs_lib::geometry::fromCV(point_in);
    geometry_msgs::Point pt_tfd;
    tf2::doTransform(pt, pt_tfd, transform);
    point_out = pairs_lib::geometry::toCV(pt_tfd);
  }
}
...
namespace pairs_lib
{
  ...
  void Transformer::setLatLon(const double lat, const double lon)
  {
    ...
    double utm_x, utm_y;
    pairs_lib::LLtoUTM(lat, lon, utm_y, utm_x, utm_zone_.data());
    got_utm_zone_ = true;
  }
  ...
  [[nodiscard]] std::optional<Eigen::Vector3d> Transformer::transformAsVector(const Eigen::Vector3d& what, const geometry_msgs::TransformStamped& tf)
  {
    ...
    const geometry_msgs::Vector3 vec = pairs_lib::geometry::fromEigenVec(what);
    const auto tfd_vec = transformImpl(tf_resolved, vec);
    if (tfd_vec.has_value())
      return pairs_lib::geometry::toEigen(tfd_vec.value());
    else
      return std::nullopt;
  }
  ...
  [[nodiscard]] std::optional<Eigen::Vector3d> Transformer::transformAsPoint(const Eigen::Vector3d& what, const geometry_msgs::TransformStamped& tf)
  {
    ...
    if (tfd_pt.has_value())
      return pairs_lib::geometry::toEigen(tfd_pt.value());
    else
      return std::nullopt;
  }
  ...
  /* specialization for pairs_msgs::ReferenceStamped //{ */
  
  std::optional<pairs_msgs::ReferenceStamped> Transformer::transformImpl(const geometry_msgs::TransformStamped& tf, const pairs_msgs::ReferenceStamped& what)
  {
    ...
    pairs_msgs::ReferenceStamped ret;
  ...
  /* specialization for pairs_msgs::Reference //{ */
  
  std::optional<pairs_msgs::Reference> Transformer::transformImpl(const geometry_msgs::TransformStamped& tf, const pairs_msgs::Reference& what)
  {
  ...
  std::optional<Eigen::Vector3d> Transformer::transformImpl(const geometry_msgs::TransformStamped& tf, const Eigen::Vector3d& what)
  {
    ...
    const geometry_msgs::Vector3 as_vec = pairs_lib::geometry::fromEigenVec(what);
  ...
  geometry_msgs::Point Transformer::LLtoUTM(const geometry_msgs::Point& what, [[maybe_unused]] const std::string& prefix)
  {
    ...
    pairs_lib::UTM(what.x, what.y, &utm.x, &utm.y);
  ...
  std::optional<geometry_msgs::Point> Transformer::UTMtoLL(const geometry_msgs::Point& what, [[maybe_unused]] const std::string& prefix)
  {
    ...
    pairs_lib::UTMtoLL(what.y, what.x, utm_zone_.data(), latlon.x, latlon.y);
    latlon.z = what.z;
    return latlon;
  }
