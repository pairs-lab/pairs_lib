#include <pairs_lib/image_publisher.h>
...
namespace pairs_lib {
  ...
  std::string ImagePublisher::getEncoding(cv::Mat& input, bool bgr_order){
    switch (input.type()){
      ...
      default:
        ROS_ERROR("[ImagePublisher]: Covnersion for the current matrix type is not yet implemented. Implement it yourself in pairs_lib/image_publisher. Sorry.");
        return "";
        break;
    }
  }
