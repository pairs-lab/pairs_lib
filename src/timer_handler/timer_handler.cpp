...
namespace pairs_lib
{
  ...
  ROSTimer::ROSTimer(const pairs_lib::TimerHandlerOptions& opts, const rclcpp::Rate& rate, const std::function<void()>& callback)
  {
  ...
  void ROSTimer::setCallback([[maybe_unused]] const std::function<void()>& callback)
  {
    ...
    RCLCPP_ERROR(node_->get_logger(), "[pairs_lib::ROSTimer]: setCallback(...) is not implemented for ROSTimer! Check [pairs_lib/src/timer/timer.cpp].");
  }
  //}
  ...
  ThreadTimer::ThreadTimer(const pairs_lib::TimerHandlerOptions& opts, const rclcpp::Rate& rate, const std::function<void()>& callback)
  {
