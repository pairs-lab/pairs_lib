...
Profiler::Profiler(ros::NodeHandle& nh, std::string _node_name_, bool profiler_enabled) {
  ...
  if (profiler_enabled) {
    ...
    publisher_       = std::make_unique<ros::Publisher>(this->nh_->advertise<pairs_msgs::ProfilerUpdate>("profiler", 100, false));
  }
...
Routine::Routine(std::string name, std::string node_name, double expected_rate, double threshold, std::shared_ptr<ros::Publisher> publisher,
  ...
  msg_out_.event     = pairs_msgs::ProfilerUpdate::START;
...

  {
    ...
    try {
      publisher_->publish(pairs_msgs::ProfilerUpdateConstPtr(new pairs_msgs::ProfilerUpdate(msg_out_)));
    }
    catch (...) {
...
void Routine::end(void) {
  ...
  msg_out_.event = pairs_msgs::ProfilerUpdate::END;
