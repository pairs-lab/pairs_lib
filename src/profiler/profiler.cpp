...
namespace pairs_lib
{
  ...
  Profiler::Profiler(const rclcpp::Node::SharedPtr& node, const std::string& node_name, const bool profiler_enabled)
  {
    ...
    if (profiler_enabled)
    {
      publisher_ = std::make_shared<pairs_lib::PublisherHandler<pairs_msgs::msg::ProfilerUpdate>>(node_, "profiler");
    }
  ...
  Routine::Routine(const rclcpp::Node::SharedPtr& node, const std::string& name, const std::string& node_name,
                   const std::shared_ptr<pairs_lib::PublisherHandler<pairs_msgs::msg::ProfilerUpdate>>& publisher, bool profiler_enabled)
  {
    ...
    msg_out_.duration = 0;
    msg_out_.event = pairs_msgs::msg::ProfilerUpdate::START;
  ...
  void Routine::end(void)
  {
    ...
    msg_out_.event = pairs_msgs::msg::ProfilerUpdate::END;
