...
/**  \file
     ...
     This example may be run after building *pairs_lib* by executing `rosrun pairs_lib repredictor_example`.
...
// |                     THE IMPORTANT PART                     |
// --------------------------------------------------------------
void process_msg(const pairs_msgs::Float64Stamped::ConstPtr msg, type_e type)
{
  ...
  pairs_msgs::Float64Stamped msg_out;
...
void callback_input(const pairs_msgs::Float64Stamped::ConstPtr msg)
{
  process_msg(msg, input);
}

void callback_meas_fast(const pairs_msgs::Float64Stamped::ConstPtr msg)
{
  process_msg(msg, meas_fast);
}

void callback_meas_slow(const pairs_msgs::Float64Stamped::ConstPtr msg)
{
...
void meas_generator_fast()
{
  ...
  while (ros::ok())
  {
    const x_t x_gt = pairs_lib::get_mutexed(x_mtx, x);
...
int main(int argc, char **argv)
{
  ...
  pub_pos_est = nh.advertise<pairs_msgs::Float64Stamped>("position_est", 5);
  pub_vel_est = nh.advertise<pairs_msgs::Float64Stamped>("velocity_est", 5);
  pub_meas_fast = nh.advertise<pairs_msgs::Float64Stamped>("position_meas_fast", 5);
  pub_meas_slow = nh.advertise<pairs_msgs::Float64Stamped>("position_meas_slow", 5);
  ros::Publisher pub_inpt_gt = nh.advertise<pairs_msgs::Float64Stamped>("velocity_inpt", 5);
  ros::Publisher pub_pos_gt = nh.advertise<pairs_msgs::Float64Stamped>("position_gt", 5);
  ros::Publisher pub_vel_gt = nh.advertise<pairs_msgs::Float64Stamped>("velocity_gt", 5);
  ...
  while (ros::ok())
  {
    ...
    const x_t x_old = pairs_lib::get_mutexed(x_mtx, x);
    const x_t x_new = generateA(dt)*x_old + generateB(dt)*u + multivariate_gaussian(dtQ);
    pairs_lib::set_mutexed(x_mtx, x_new, x);
