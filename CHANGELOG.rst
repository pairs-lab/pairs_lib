^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package pairs_lib
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

1.0.4 (2023-01-20)
------------------
* Remove all warnings (Eigen and pcl) (`#40 <https://github.com/ctu-mrs/pairs_lib/issues/40>`_)
...
* geometry updates (`#38 <https://github.com/ctu-mrs/pairs_lib/issues/38>`_)
...
* [MedianFilter]: rewrote the implementation to be clearer and consistent with the rest of pairs_lib, added documentation and tests
...
* scope_timer: add ScopeTimerLogger; use chrono::steady_clock insteady of chrono::system_clock (`#34 <https://github.com/ctu-mrs/pairs_lib/issues/34>`_)
...
* fixed an error in transformer: the last message was published with ros::Time::now() time stamp, not with it's real stamp. (`#32 <https://github.com/ctu-mrs/pairs_lib/issues/32>`_)
...
* [timer]: added more tests, fixes in tests and implementation (`#31 <https://github.com/ctu-mrs/pairs_lib/issues/31>`_)
  hopefully this will fix stuff
* fixed transformer logging
* [cyclic] added comparison and ostream operators
* Enable SubscribeHandler to use either ROSTimer or ThreadTimer (`#30 <https://github.com/ctu-mrs/pairs_lib/issues/30>`_)
...
* service client handler not dependent on pairs_lib anymore
...
* [transformer]: added transformVecs for transforming Eigen stuff and getTransformEigen to pairs_lib::TransformStamped
* [transformer]: added the inverse() method to pairs_lib::TransformStamped
...
* pairs_lib: removed caching, fixed name resolver
