...
namespace pairs_lib
{
  ...
  */
  struct SubscribeHandlerOptions
  {
    ...
    std::shared_ptr<pairs_lib::TimeoutManager> timeout_manager = nullptr;  /*!< \brief Will be used for handling message timouts if necessary. If no manager is specified, it will be created with rate equal to half of \p no_message_timeout. */
  
    ros::Duration no_message_timeout = pairs_lib::no_timeout;  /*!< \brief If no new message is received for this duration, the \p timeout_callback function will be called. If \p timeout_callback is empty, an error message will be printed to the console. */
  ...
  class SubscribeHandler
  {
    ...
    /*!
      ...
      */
      ...
      SubscribeHandler(
            const SubscribeHandlerOptions& options,
            pairs_lib::TimeoutManager& timeout_manager,
            Types ... args
          )
      :
        SubscribeHandler(
...
#include <pairs_lib/impl/subscribe_handler.hpp>
