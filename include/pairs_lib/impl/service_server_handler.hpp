...
namespace pairs_lib
{
  ...
  ServiceServerHandler<ServiceType>::ServiceServerHandler(
      ...
      pairs_lib::Task<bool> (ClassType::*method)(const std::shared_ptr<typename ServiceType::Request> request,
      ...
      if (!was_running)
      {
        internal::start_task(
            [](std::shared_ptr<typename rclcpp::Service<ServiceType>::SharedPtr> server, // Pass the captured safe pointer
               ...
               pairs_lib::Task<bool> (ClassType::*method)(const std::shared_ptr<typename ServiceType::Request>,
                                                        const std::shared_ptr<typename ServiceType::Response>),
               ClassType* instance, std::shared_ptr<rmw_request_id_t> req_id, std::shared_ptr<typename ServiceType::Request> req) -> pairs_lib::Task<void> {
