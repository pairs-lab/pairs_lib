          ...
          m_transport_hints(options.transport_hints)
    {
      ...
      if (options.no_message_timeout != pairs_lib::no_timeout)
      {
        ...
        if (!m_timeout_manager)
          m_timeout_manager = std::make_shared<pairs_lib::TimeoutManager>(m_nh, ros::Rate(options.no_message_timeout * 0.5));
    ...
    virtual void setNoMessageTimeout(const ros::Duration& timeout)
    {
      ...
      else
      {
        ...
        if (m_timeout_manager == nullptr)
          m_timeout_manager = std::make_shared<pairs_lib::TimeoutManager>(m_nh, ros::Rate(timeout * 0.5));
