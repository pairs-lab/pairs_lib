...
namespace pairs_lib
{
  ...
  public:
          ...
          m_latest_message(nullptr), m_message_callback(message_callback)
    {
      ...
      if (options.no_message_timeout != pairs_lib::no_timeout)
      {
        ...
        {
          m_timeout_manager =
              std::make_shared<pairs_lib::TimeoutManager>(m_node, rclcpp::Rate(1.0 / (options.no_message_timeout.seconds() * 0.5), m_node->get_clock()));
        }
  ...
  public:
    ...
    {
      if (timeout == pairs_lib::no_timeout)
      {
      ...
      } else
      {
        ...
        if (m_timeout_manager == nullptr)
          m_timeout_manager = std::make_shared<pairs_lib::TimeoutManager>(m_node, rclcpp::Rate(1.0 / (timeout.seconds() * 0.5), m_node->get_clock()));
  ...
  protected:
    std::shared_ptr<pairs_lib::TimeoutManager> m_timeout_manager;
    std::optional<pairs_lib::TimeoutManager::timeout_id_t> m_timeout_id;
    pairs_lib::TimeoutManager::callback_t m_timeout_mgr_callback;
