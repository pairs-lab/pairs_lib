...
namespace pairs_lib
{
  ...
  struct SubscriberHandlerOptions
  {
    ...
    std::shared_ptr<pairs_lib::TimeoutManager> timeout_manager = nullptr; /*!< \brief Will be used for handling message timouts if necessary. If no manager is
                                                                           specified, it will be created with rate equal to half of \p no_message_timeout. */

    rclcpp::Duration no_message_timeout =
        pairs_lib::no_timeout; /*!< \brief If no new message is received for this duration, the \p timeout_callback function will
  ...
  public:
    ...
    SubscriberHandler(const SubscriberHandlerOptions& options, pairs_lib::Task<> (ObjectType2::*const message_callback)(typename MessageType::ConstSharedPtr),
     ...
     */
    template <class... Types>
    SubscriberHandler(const SubscriberHandlerOptions& options, std::shared_ptr<pairs_lib::TimeoutManager>& timeout_manager, Types... args)
        ...
        if (!was_running)
        {
          internal::start_task(
              [](std::shared_ptr<std::atomic<bool>> is_running, Task<> (C::*method)(typename MessageType::ConstSharedPtr msg), C* instance,
                 typename MessageType::ConstSharedPtr msg) -> pairs_lib::Task<void> {
