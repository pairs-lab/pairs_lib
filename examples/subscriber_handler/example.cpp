...
/**  \file
     ...
     This example may be run after building *pairs_lib* by executing `rosrun pairs_lib subscribe_handler_example`.
...
int main(int argc, char** argv)
{
  ...
  pairs_lib::SubscriberHandlerOptions shopts(node);
  ...
  pairs_lib::SubscriberHandler<std_msgs::msg::String> handler(shopts, topic_name, std::bind(timeout_callback, node, std::placeholders::_1, std::placeholders::_2),
                                                            message_callback);

  /* A variation of the factory method for easier use with objects also exists. */
  pairs_lib::construct_object(handler, shopts, topic_name, no_message_timeout,
  ...
  using message_type = pairs_lib::message_type<decltype(handler)>;
