...
int main(int argc, char* argv[])
{
  ...
  ros::NodeHandle nh("~");
  pairs_lib::Transformer tfr(nh);

  const std::string from = pairs_lib::LATLON_ORIGIN;
  const std::string to = pairs_lib::UTM_ORIGIN;
  ...
  while (ros::ok())
  {
    ...
    if (tf_opt.has_value())
    {
      ...
      std::cout << "Received transformation from \"" << pairs_lib::Transformer::frame_from(tf) << "\" to \"" << pairs_lib::Transformer::frame_to(tf) << "\":\n";
