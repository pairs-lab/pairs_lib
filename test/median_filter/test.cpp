...
TEST(TESTSuite, constraints)
{
  ...
  pairs_lib::MedianFilter fil(bfr_len, min, max, max_diff);
...
{
  constexpr size_t bfr_len = 5;
  pairs_lib::MedianFilter fil(bfr_len);
...
TEST(TESTSuite, copy)
{
  ...
  {
    // copy constructor
    pairs_lib::MedianFilter fil2 = fil;
  ...
  {
    // move constructor
    pairs_lib::MedianFilter fil2(std::move(fil));
    EXPECT_TRUE(fil2.full());
    EXPECT_EQ(fil2.median(), 3);
  }
