...
TEST(TESTSuite, point_valid_check)
{
  ...
  auto border = std::make_unique<pairs_lib::safety_zone::Prism>(points, max_z, min_z, "world_origin", "world_origin");

  auto safety_zone = std::make_shared<pairs_lib::safety_zone::SafetyZone>(std::move(border));
...
TEST(TESTSuite, obstacle_valid_check)
{
  ...
  std::vector<pairs_lib::safety_zone::Point2d> border_points;
  border_points.reserve(border_matrix.rows());

  for (int i = 0; i < border_matrix.rows(); i++)
  {
    border_points.emplace_back(pairs_lib::safety_zone::Point2d{border_matrix(i, 0), border_matrix(i, 1)});
  }

  double border_max_z = 5.0;
  double border_min_z = 0.0;

  auto border = std::make_unique<pairs_lib::safety_zone::Prism>(border_points, border_max_z, border_min_z, "world_origin", "world_origin");
  ...
  std::vector<pairs_lib::safety_zone::Point2d> obstacle_points;
  obstacle_points.reserve(obstacle_matrix.rows());

  for (int i = 0; i < obstacle_matrix.rows(); i++)
  {
    obstacle_points.emplace_back(pairs_lib::safety_zone::Point2d{obstacle_matrix(i, 0), obstacle_matrix(i, 1)});
  }

  double obstacle_max_z = 3.0;
  double obstacle_min_z = 1.0;

  auto obstacle = std::make_unique<pairs_lib::safety_zone::Prism>(obstacle_points, obstacle_max_z, obstacle_min_z, "world_origin", "world_origin");

  std::vector<std::unique_ptr<pairs_lib::safety_zone::Prism>> obstacles;
  obstacles.push_back(std::move(obstacle));

  auto safety_zone = std::make_shared<pairs_lib::safety_zone::SafetyZone>(std::move(border), std::move(obstacles));
