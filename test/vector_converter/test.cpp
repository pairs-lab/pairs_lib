...
#include <pairs_lib/impl/vector_converter_types.h>
#include <iostream>
...
{
  [[maybe_unused]] const TypeParam vec = pairs_lib::impl::convertTo<TypeParam>(rand(), rand(), rand());;
}

TYPED_TEST_P(VectorConverterTest, ConvertsFrom)
{
  const TypeParam vec;
  [[maybe_unused]] const auto ret = pairs_lib::impl::convertFrom(vec);
}

TYPED_TEST_P(VectorConverterTest, ConvertsValid)
{
  const double xo = rand(), yo = rand(), zo = rand();
  const TypeParam vec = pairs_lib::impl::convertTo<TypeParam>(xo, yo, zo);;
  const auto [x, y, z] = pairs_lib::impl::convertFrom(vec);
...
{                                                                   
  const double xo = rand(), yo = rand(), zo = rand();               
  const from_t from = pairs_lib::impl::convertTo<from_t>(xo, yo, zo); 
  const to_t to = pairs_lib::convert<to_t>(from);                     
  const auto [x, y, z] = pairs_lib::impl::convertFrom(to);            
  return                                                            
      (x == xo && y == yo && z == zo)                               
