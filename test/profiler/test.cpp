...
public:
  ...
  void callback1(pairs_msgs::msg::ProfilerUpdate::ConstSharedPtr msg)
  {
...
TEST_F(Test, test_basic)
{
  ...
  profiler_ = pairs_lib::Profiler(node_, "Test", true);

  clock->sleep_for(0.1s);

  // | ------------------- create a subscriber ------------------ |

  const std::function<void(const pairs_msgs::msg::ProfilerUpdate::SharedPtr)> callback1_ptr = std::bind(&Test::callback1, this, std::placeholders::_1);

  auto sub1 = node_->create_subscription<pairs_msgs::msg::ProfilerUpdate>("profiler", 100, callback1_ptr);
...
TEST_F(Test, test_not_active)
{
  ...
  profiler_ = pairs_lib::Profiler(node_, "Test", false);
