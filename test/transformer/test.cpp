...
using namespace std;
using namespace pairs_lib::geometry;
...
std::optional<geometry_msgs::TransformStamped> wait_for_tf(const std::string& from, const std::string& to, pairs_lib::Transformer& tfr, const ros::Time& publish_t = ros::Time::now(), const ros::Time& stamp = ros::Time(0))
{
...
TEST(TESTSuite, main_test)
{
  ...
  auto tfr = pairs_lib::Transformer("Transformer_main_test");
...
TEST(TESTSuite, tf_times_test)
{
  ...
  auto tfr = pairs_lib::Transformer("Transformer_tf_times_test");
  ...
  tf.transform.translation = pairs_lib::geometry::fromEigenVec(common2uav1.translation());
  bc->sendTransform(tf);
  ros::spinOnce();
  ...
  tf.transform.translation = pairs_lib::geometry::fromEigenVec(common2uav2.translation());
  bc->sendTransform(tf);
  ros::spinOnce();
...
TEST(TESTSuite, eigen_vector3d_test)
{
  ...
  auto tfr = pairs_lib::Transformer("Transformer_eigen_vector3d_test");
  tfr.setDefaultPrefix("uav66");
  ...
  if (tf_opt.has_value()) {
    ...
    for (const auto& tv : test_vectors) {
      ...
      // | ----------------------- as a vector ---------------------- |
      {
        const auto rv = tfr.transformAsVector(pairs_lib::Transformer::frame_from(tf), tv, pairs_lib::Transformer::frame_to(tf), tf.header.stamp);
        if (!rv)
        {
          ROS_ERROR_STREAM_THROTTLE(1.0, "[" << ros::this_node::getName() << "]: Failed to transform vector [" << tv.transpose() << "]");
          result *= 0;
      ...
      // | ----------------------- as a point ----------------------- |
      {
        const auto rv = tfr.transformAsPoint(pairs_lib::Transformer::frame_from(tf), tv, pairs_lib::Transformer::frame_to(tf), tf.header.stamp);
        if (!rv)
        {
          ROS_ERROR_STREAM_THROTTLE(1.0, "[" << ros::this_node::getName() << "]: Failed to transform vector [" << tv.transpose() << "]");
          result *= 0;
...
TEST(TESTSuite, transform_single)
{
  ...
  auto tfr = pairs_lib::Transformer("Transformer_transform_single");
  tfr.setDefaultPrefix("uav66");
  ...
  // test transforming a pairs_msgs::Reference
  {
...
TEST(TESTSuite, pairs_reference_test)
{
  ...
  if (tf_opt.has_value()) {
    ...
    std::vector<std::pair<pairs_msgs::ReferenceStamped, pairs_msgs::ReferenceStamped>> test_refs;
    pairs_msgs::ReferenceStamped orig, tfd;
    ...
    for (const auto& tv : test_refs) {
      ...
      if (!rv_opt) {
        ...
        const Eigen::Vector3d vect_diff = pairs_lib::geometry::toEigen(rv.reference.position) - pairs_lib::geometry::toEigen(gt.reference.position);
...
TEST(TESTSuite, quaternion_test)
{
  ...
  auto tfr = pairs_lib::Transformer("Transformer_quaternion_test");
  tfr.setDefaultPrefix("uav66");
...
TEST(TESTSuite, opencv_test)
{
  ...
  auto tfr = pairs_lib::Transformer("Transformer_opencv_test");
  tfr.setDefaultPrefix("uav66");
...
bool compare_gt_latlon(const geometry_msgs::Point& tv, const geometry_msgs::Point& rv, const char utm_zone[10], const bool ll2local)
{
  ...
  if (ll2local)
  {
    ...
    Eigen::Vector3d utm;
    pairs_lib::UTM(tv.x, tv.y, &(utm.x()), &(utm.y()));
  ...
  {
    const vec3_t utm = local2utm*pairs_lib::geometry::toEigen(tv);
    Eigen::Vector3d latlon;
    pairs_lib::UTMtoLL(utm.y(), utm.x(), utm_zone, latlon.x(), latlon.y());
    latlon.z() = utm.z();
    gt = latlon;
  }

  const Eigen::Vector3d vect_diff = pairs_lib::geometry::toEigen(rv) - gt;
  if (vect_diff.norm() > 1e-6)
  {
...
bool compare_gt_latlon(const pairs_msgs::Reference& tv, const pairs_msgs::Reference& rv, const char utm_zone[10], const bool ll2local)
{
  vec3_t gt;
  if (ll2local)
  {
    // convert LAT-LON to UTM
    Eigen::Vector3d utm;
    pairs_lib::UTM(tv.position.x, tv.position.y, &(utm.x()), &(utm.y()));
    // copy the height from the input
    utm.z() = tv.position.z;
    const vec3_t local = local2utm.inverse()*utm;
    gt = local;
  ...
  {
    const vec3_t utm = local2utm*pairs_lib::geometry::toEigen(tv.position);
    Eigen::Vector3d latlon;
    pairs_lib::UTMtoLL(utm.y(), utm.x(), utm_zone, latlon.x(), latlon.y());
    latlon.z() = utm.z();
    gt = latlon;
  ...
  const Eigen::Vector3d vect_diff = pairs_lib::geometry::toEigen(rv.position) - gt;
  if (vect_diff.norm() > 1e-6)
  {
    ROS_ERROR_STREAM("<< Transformed [" << tv << "] with value of ["
                                       << rv << "] does not match the expected value of [" << gt.transpose() << "]");
...
template <typename T>
bool trytransform_latlon(const T& tv, pairs_lib::Transformer& tfr, const char utm_zone[10], const bool ll2local, const bool expect_ok)
{
  const std::string local = "local_origin";
  const std::string ll = pairs_lib::LATLON_ORIGIN;
...
TEST(TESTSuite, latlon_test)
{
  ...
  auto tfr = pairs_lib::Transformer("Transformer_latlon_test");
  tfr.setDefaultPrefix("uav66");
  ...
  double utm_x, utm_y;
  char utm_zone_[10];
  pairs_lib::LLtoUTM(lat, lon, utm_y, utm_x, utm_zone_);
...
TEST(TESTSuite, eigen_decomposed_test)
{
  ...
  auto tfr = pairs_lib::Transformer("Transformer_eigen_decomposed_test");
  tfr.setDefaultPrefix("uav66");
...
TEST(TESTSuite, retry_latest_test)
{
  ...
  auto tfr = pairs_lib::Transformer("Transformer_retry_latest_test");
  tfr.setDefaultPrefix("uav66");
...
TEST(TESTSuite, prefix_test)
{
  ...
  auto tfr = pairs_lib::Transformer("Transformer_prefix_test");
...
TEST(TESTSuite, default_frame_test)
{
  ...
  auto tfr = pairs_lib::Transformer("Transformer_default_frame_test");
  tfr.setDefaultPrefix("uav66");
