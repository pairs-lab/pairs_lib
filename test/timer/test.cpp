...
struct obj_t
{
  ...
  void callback(const ros::TimerEvent& evt)
  {
    ...
    pairs_lib::AtomicScopeFlag running(cbk_running);
    n_cbks++;
...
void do_test(const bool use_threadtimer)
{
  ...
  if (use_threadtimer)
    timer = std::make_unique<pairs_lib::ThreadTimer>(nh, cbk_obj.r, &obj_t::callback, &cbk_obj, false, false);
  else
    timer = std::make_unique<pairs_lib::ROSTimer>(nh, cbk_obj.r, &obj_t::callback, &cbk_obj, false, false);
