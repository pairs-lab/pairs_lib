...
protected:
  ...
  std::shared_ptr<pairs_lib::TimeoutManager> tom_;
...
struct obj_t
{
  ...
  pairs_lib::TimeoutManager::timeout_id_t timeout_id;

  obj_t(rclcpp::Node::SharedPtr node, std::shared_ptr<pairs_lib::TimeoutManager> tom, std::shared_ptr<std::atomic<bool>> test_stop_from_cbk,
  ...
  void set_timeout_id(pairs_lib::TimeoutManager::timeout_id_t new_timeout_id)
  {
  ...
  void callback(const rclcpp::Time& last_update)
  {
    ...
    pairs_lib::AtomicScopeFlag flg(cbk_running);
    std::scoped_lock lck(mtx);
...
TEST_F(Test, test_timeout_manager)
{
  ...
  //
  //  pairs_lib.Test test_timeout_manager (/tmp/workspace/src/pairs_lib/test/timeout_manager/test.cpp:194)
  //  <<< failure message
  //    /tmp/workspace/src/pairs_lib/test/timeout_manager/test.cpp:319
  ...
  tom_ = std::make_unique<pairs_lib::TimeoutManager>(node_, rclcpp::Rate(1.0 / update_period, clock));
  ...
  for (int it = 0; it < 4; it++)
  {
    ...
    pairs_lib::TimeoutManager::callback_t cbk = std::bind(&obj_t::callback, &objs[it], std::placeholders::_1);
