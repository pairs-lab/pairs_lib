...
std::unique_ptr<pairs_lib::TimeoutManager> tom = nullptr;
...
TEST(TESTSuite, timeout_test)
{
  ...
  //
  //  [pairs_lib.rosunit-timeout_manager/timeout_test][FAILURE]-------------------------
  // /tmp/workspace/src/pairs_lib/test/timeout_manager/test.cpp:188
  ...
  tom = std::make_unique<pairs_lib::TimeoutManager>(*nh, ros::Rate(update_period));
