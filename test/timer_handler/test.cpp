...
protected:
  ...
  pairs_lib::TimerHandlerOptions default_options()
  {
  ...
  void initializeTimer(std::optional<pairs_lib::TimerHandlerOptions> opts_arg = {})
  {
    RCLCPP_INFO(node_->get_logger(), "Constructing timer");

    pairs_lib::TimerHandlerOptions opts = opts_arg.value_or(default_options());
    ...
    case TestTimerType::ros:
      timer_ = std::make_shared<pairs_lib::ROSTimer>(opts, rclcpp::Rate(rate_, node_->get_clock()), callback_fcn);
      break;
    case TestTimerType::thread:
      timer_ = std::make_shared<pairs_lib::ThreadTimer>(opts, rclcpp::Rate(rate_, node_->get_clock()), callback_fcn);
      break;
    default:
...
public:
  ...
  std::shared_ptr<pairs_lib::MRSTimer> create_timer(const pairs_lib::TimerHandlerOptions& opts, const rclcpp::Rate& rate)
  {
    ...
    {
    case TestTimerType::ros:
      return std::make_shared<pairs_lib::ROSTimer>(opts, rate, [this]() { callback(); });
    case TestTimerType::thread:
      return std::make_shared<pairs_lib::ThreadTimer>(opts, rate, [this]() { callback(); });
    default:
      throw std::logic_error("Unhandled timer type");
    }
  }
