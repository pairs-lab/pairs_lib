...
TEST(TESTSuite, rpy_settters_getters) {
  ...
  tf2::Quaternion quat_from_ext = AttitudeConverter(e_roll, e_pitch, e_yaw, pairs_lib::RPY_EXTRINSIC);  // the default option without the argument
  tf2::Quaternion quat_from_int = AttitudeConverter(i_roll, i_pitch, i_yaw, pairs_lib::RPY_INTRINSIC);
...

  {
    ...
    Eigen::Matrix3d ref_R = pairs_lib::AttitudeConverter(quat);
    ...
    Eigen::Matrix3d ref_ext_R = pairs_lib::AttitudeConverter(roll, pitch, yaw, pairs_lib::RPY_EXTRINSIC);
    ...
    Eigen::Matrix3d ref_int_R = pairs_lib::AttitudeConverter(roll, pitch, yaw, pairs_lib::RPY_INTRINSIC);
    std::cout << "ref throught int R = " << std::endl << ref_int_R << std::endl;
    ;

    x_vec = AttitudeConverter(roll, pitch, yaw, pairs_lib::RPY_INTRINSIC).getVectorX();
    y_vec = AttitudeConverter(roll, pitch, yaw, pairs_lib::RPY_INTRINSIC).getVectorY();
    z_vec = AttitudeConverter(roll, pitch, yaw, pairs_lib::RPY_INTRINSIC).getVectorZ();
...
TEST(TESTSuite, get_heading_exception) {
  ...
  catch (const pairs_lib::AttitudeConverter::GetHeadingException& e) {
    result = 1;
...

  {
    ...
    for (int i = 0; i < 1000; i++) {
...

      {
        ...
        catch (const pairs_lib::AttitudeConverter::SetHeadingException& e) {
          continue;
        }
...
TEST(TESTSuite, get_heading_rate) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    double heading_difference = pairs_lib::geometry::sradians::diff(new_heading, heading);
...
TEST(TESTSuite, input_nan_checks) {
  ...
  catch (const pairs_lib::AttitudeConverter::InvalidAttitudeException& e) {
