...
namespace
{
  ...
  static_assert(noexcept(std::declval<pairs_lib::internal::ResultStorage<int>>().set_value(10)),
                "ResultStorage::set_value should be noexcept when stored type is noexcept move constructible");
  static_assert(noexcept(std::declval<pairs_lib::internal::ResultStorage<std::string>>().set_value(std::declval<std::string>())),
                "ResultStorage::set_value should be noexcept when stored type is noexcept move constructible");
  static_assert(!noexcept(std::declval<pairs_lib::internal::ResultStorage<PotentiallyThrowingMoveOnly>>().set_value(std::declval<PotentiallyThrowingMoveOnly>())),
                "ResultStorage::set_value should NOT be noexcept when stored type is NOT noexcept move constructible");

  static_assert(noexcept(std::declval<pairs_lib::internal::ResultStorage<int>>().set_exception(std::declval<std::exception_ptr>())),
                "ResultStorage::set_exception should be noexcept");
  static_assert(noexcept(std::declval<pairs_lib::internal::ResultStorage<std::string>>().set_exception(std::declval<std::exception_ptr>())),
                "ResultStorage::set_exception should be noexcept");
  static_assert(noexcept(std::declval<pairs_lib::internal::ResultStorage<PotentiallyThrowingMoveOnly>>().set_exception(std::declval<std::exception_ptr>())),
                "ResultStorage::set_exception should be noexcept");

  consteval bool result_storage_test_default_ctor_dtor()
  {
    pairs_lib::internal::ResultStorage<int> storage;
  ...
  {
    std::string s = "asdf";
    pairs_lib::internal::ResultStorage<std::string> storage;
...
namespace
{
  ...
  pairs_lib::Task<> co_takes_val(int)
  {
    co_return;
  };

  pairs_lib::Task<> co_takes_lval_ref(int&)
  {
    co_return;
  };

  pairs_lib::Task<> co_takes_rval_ref(int&&)
  {
    co_return;
  };

  pairs_lib::Task<> co_takes_move_only(std::unique_ptr<int>)
  {
    co_return;
  };

  pairs_lib::Task<> co_takes_move_only_lval_ref(std::unique_ptr<int>&)
  {
    co_return;
  };

  pairs_lib::Task<> co_takes_move_only_rval_ref(std::unique_ptr<int>&&)
  {
    co_return;
  };

  pairs_lib::Task<> co_set_42(int& out)
  {
    out = 42;
    co_return;
  }

  pairs_lib::Task<int> co_42()
  {
    co_return 42;
  }

  pairs_lib::Task<std::unique_ptr<int>> co_uptr_42()
  {
    auto val = std::make_unique<int>(42);
    co_return val;
  }

  pairs_lib::Task<int> co_2_times_42()
  {
  ...
  pairs_lib::Task<> co_throws_logic_error()
  {
    throw_expected_exception();
    co_return;
  }

  pairs_lib::Task<> co_call_throwning(size_t levels)
  {
    if (levels == 0)
    {
  ...
  pairs_lib::Task<int> co_throws_logic_error_non_void()
  {
    throw_expected_exception();
    co_return 42;
  }

  pairs_lib::Task<int> co_call_throwning_non_void(size_t levels)
  {
    if (levels == 0)
    {
  ...
  template <typename R, typename... FuncArgs, typename... Args>
    requires std::invocable<pairs_lib::Task<R> (*)(FuncArgs...), Args...>
  pairs_lib::Task<bool> co_check_throws(pairs_lib::Task<R> (*coroutine)(FuncArgs...), Args&&... args)
  {
  ...
  pairs_lib::Task<ErrorOnMove> co_get_error_on_move()
  {
    co_return ErrorOnMove{};
  }


  pairs_lib::Task<std::exception_ptr> co_get_exception_ptr()
  {
    co_return std::make_exception_ptr(std::logic_error(std::string(test_not_thrown_exception_message)));
  }

  pairs_lib::Task<bool> check_get_exception_ptr()
  {
  ...
  TEST(PairsLibCoro, StartTaskArgsValueCategories)
  {
    ...
    // function reference
    pairs_lib::internal::start_task(co_takes_val, 42);
    pairs_lib::internal::start_task(co_takes_lval_ref, std::ref(lval_int));
    pairs_lib::internal::start_task(co_takes_rval_ref, 42);
    pairs_lib::internal::start_task(co_takes_move_only, std::make_unique<int>(42));
    pairs_lib::internal::start_task(co_takes_move_only_lval_ref, std::ref(lval_uptr));
    pairs_lib::internal::start_task(co_takes_move_only_rval_ref, std::make_unique<int>(42));
    // function pointer
    pairs_lib::internal::start_task(&co_takes_val, 42);
    pairs_lib::internal::start_task(&co_takes_lval_ref, std::ref(lval_int));
    pairs_lib::internal::start_task(&co_takes_rval_ref, 42);
  ...
  TEST(PairsLibCoro, StartCopyDecays)
  {
    struct Func
    {
      pairs_lib::Task<> operator()(int&& arg)
      {
        arg = 24;
        co_return;
      }
    ...
    pairs_lib::internal::start_task(Func{}, val);

    EXPECT_EQ(val, 0);

    // The reference wrapper should be able to keep the reference and call the `int&` overload
    pairs_lib::internal::start_task(Func{}, std::ref(val));

    EXPECT_EQ(val, 42);
  }

  TEST(PairsLibCoro, StartTaskLambda)
  {
    bool finished = false;

    pairs_lib::internal::start_task(
        [](bool& finished) -> pairs_lib::Task<> {
          finished = true;
          co_return;
        },
        std::ref(finished));
  ...
  TEST(PairsLibCoro, StartTaskFuncRef)
  {
    int result = 0;

    // No ampersand in front of function name - passing function reference
    pairs_lib::internal::start_task(co_set_42, std::ref(result));

    EXPECT_EQ(result, 42);
  }

  TEST(PairsLibCoro, StartTaskFuncPtr)
  {
    int result = 0;
  ...
  TEST(PairsLibCoro, ReturnUniquePtr)
  {
    bool finished = false;
  ...
  TEST(PairsLibCoro, ChainedCoroutines)
  {
    bool finished = false;
  ...
  TEST(PairsLibCoro, ExceptionPropagationVoid)
  {
    bool finished = false;
  ...
  TEST(PairsLibCoro, MultiLevelExceptionPropagationVoid)
  {
    for (size_t level = 1; level < 10; level++)
    {
      ...
      pairs_lib::internal::start_task(
          [](bool& finished, size_t level) -> pairs_lib::Task<> {
  ...
  TEST(PairsLibCoro, ExceptionPropagationNonVoid)
  {
    bool finished = false;
  ...
  TEST(PairsLibCoro, MultiLevelExceptionPropagationNonVoid)
  {
    for (size_t level = 1; level < 10; level++)
    {
      bool finished = false;
  ...
  TEST(PairsLibCoro, ErrorOnReturn)
  {
    bool finished = false;
  ...
  TEST(PairsLibCoro, ReturnExceptionPtr)
  {
    bool finished = false;
  ...
  class PairsLibCoroLoops : public ::testing::TestWithParam<size_t>
  {
  };

  pairs_lib::Task<size_t> co_get_1()
  {
    co_return 1;
  }
  ...
  pairs_lib::Task<size_t> co_test_long_loop(size_t iterations)
  {
    size_t sum = 0;
  ...
  TEST_P(PairsLibCoroLoops, LongLoop)
  {
    ...
    pairs_lib::internal::start_task(
        [](bool& finished, size_t iterations) -> pairs_lib::Task<> {
  ...
  INSTANTIATE_TEST_SUITE_P(I, PairsLibCoroLoops, ::testing::Values(1'000, 10'000, 100'000, 1'000'000, 10'000'000, 100'000'000),
                           [](const testing::TestParamInfo<PairsLibCoroLoops::ParamType>& info) { return std::format("{}_iters", info.param); });
