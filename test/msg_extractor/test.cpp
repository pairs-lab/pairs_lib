...
TEST(TESTSuite, point_position)
{
  ...
  auto [x, y, z] = pairs_lib::getXYZ(msg);
...
TEST(TESTSuite, pose)
{
  ...
  msg.orientation = pairs_lib::AttitudeConverter(0.1, 0.2, 0.3).setHeading(0.1);

  auto [x, y, z] = pairs_lib::getPosition(msg);

  EXPECT_EQ(x, 1);
  EXPECT_EQ(y, 2);
  EXPECT_EQ(z, 3);

  double heading = pairs_lib::getHeading(msg);

  EXPECT_NEAR(heading, 0.1, 1e-3);

  msg.orientation = pairs_lib::AttitudeConverter(0.1, 0.2, 0.3).setYaw(0.2);

  double yaw = pairs_lib::getYaw(msg);
...
TEST(TESTSuite, pose_with_covariance)
{
  ...
  msg.pose.orientation = pairs_lib::AttitudeConverter(0.1, 0.2, 0.3).setHeading(0.1);
  ...
  msg.pose.orientation = pairs_lib::AttitudeConverter(0.1, 0.2, 0.3).setYaw(0.2);
...
TEST(TESTSuite, twist)
{
  ...
  auto [x, y, z] = pairs_lib::getVelocity(msg);
...
TEST(TESTSuite, odometry)
{
  ...
  msg.pose.pose.orientation = pairs_lib::AttitudeConverter(0.1, 0.2, 0.3).setHeading(0.1);
  ...
  msg.pose.pose.orientation = pairs_lib::AttitudeConverter(0.1, 0.2, 0.3).setYaw(0.2);
...
TEST(TESTSuite, tracker_command)
{
  ...
  pairs_msgs::msg::TrackerCommand msg;
...

  {
    ...
    msg.orientation = pairs_lib::AttitudeConverter(0.1, 0.2, 0.3).setHeading(0.15);
...
TEST(TESTSuite, tracker_command_ptr)
{
  ...
  pairs_msgs::msg::TrackerCommand::SharedPtr msg = std::make_shared<pairs_msgs::msg::TrackerCommand>();
...
TEST(TESTSuite, reference_ptr)
{
  ...
  pairs_msgs::msg::Reference::SharedPtr msg = std::make_shared<pairs_msgs::msg::Reference>();
...
TEST(TESTSuite, reference_stamped_ptr)
{
  ...
  pairs_msgs::msg::ReferenceStamped::SharedPtr msg = std::make_shared<pairs_msgs::msg::ReferenceStamped>();
