...
using namespace pairs_lib::errorgraph;
...
protected:
  void SetUp() override
  {
    ...
    sub_ = node_->create_subscription<pairs_msgs::msg::ErrorgraphElement>("~/errors", 10, [this](const pairs_msgs::msg::ErrorgraphElement::SharedPtr msg) {
  ...
  std::vector<pairs_msgs::msg::ErrorgraphElement> getReceivedMessages()
  {
    std::scoped_lock lck(mtx_);
  ...
  std::mutex mtx_;
  std::vector<pairs_msgs::msg::ErrorgraphElement> received_msgs_;
};
...
TEST_F(ErrorPublisherTest, add_waiting_for_node_error_deduplicates)
{
  ...
  for (const auto& msg : msgs)
  {
    ...
    for (const auto& error : msg.errors)
    {
      if (error.type == pairs_msgs::msg::ErrorgraphError::TYPE_WAITING_FOR_NODE && error.waited_for_node.node == "dep_node")
        count++;
    }
    if (count > 0)
    {
...
TEST_F(ErrorPublisherTest, add_waiting_for_topic_error)
{
  ...
  for (const auto& msg : msgs)
  {
    for (const auto& error : msg.errors)
    {
      if (error.type == pairs_msgs::msg::ErrorgraphError::TYPE_WAITING_FOR_TOPIC)
      {
...
TEST_F(ErrorPublisherTest, add_waiting_for_topic_error_deduplicates)
{
  ...
  for (const auto& msg : msgs)
  {
    ...
    for (const auto& error : msg.errors)
    {
      if (error.type == pairs_msgs::msg::ErrorgraphError::TYPE_WAITING_FOR_TOPIC && error.waited_for_topic == "/dup/topic")
        count++;
    }
    if (count > 0)
    {
