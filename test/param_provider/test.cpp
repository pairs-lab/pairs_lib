...
TEST_F(Test, param_provider_declare)
{
  ...
  auto pp = pairs_lib::ParamProvider(node_, true);
...
TEST_F(Test, param_provider_load)
{
  ...
  auto param_provider = pairs_lib::ParamProvider(node_, false);
...
TEST_F(Test, param_provider_set)
{
  ...
  pairs_lib::ParamProvider::range_t<int> range = {.minimum = -1, .maximum = 100};
...
TEST_F(Test, param_provider_get_param_result)
{
  ...
  auto pp = pairs_lib::ParamProvider(node_, false);
  EXPECT_TRUE(pp.addYamlFile(test_resources_path.string() + "/test_config.yaml"));

  int loaded_value = 0;
  EXPECT_EQ(pp.getParamResult("param_provider/test_int", loaded_value), pairs_lib::ParamProvider::get_result_t::LOADED);
  EXPECT_EQ(loaded_value, 666);

  int defaulted_value = 0;
  const auto default_opts = pairs_lib::ParamProvider::get_options_t<int>{.declare_options = {.default_value = 42}};
  EXPECT_EQ(pp.getParamResult("param_provider/nonexistent_param", defaulted_value, default_opts), pairs_lib::ParamProvider::get_result_t::DEFAULT);
  ...
  int missing_value = -1;
  EXPECT_EQ(pp.getParamResult("param_provider/nonexistent_param_without_default", missing_value), pairs_lib::ParamProvider::get_result_t::FAILED);
