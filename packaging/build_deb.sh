...
#   /src     -> the catkin src/ directory containing pairs_lib/ and pairs_msgs/
...
OS_NAME="ubuntu"
OS_VERSION="focal"     # Noetic targets Ubuntu 20.04 (focal)
...
# Noetic is EOL, so it is skipped by the default rosdep index; --include-eol-distros
# keeps its keys (roscpp, cmake_modules, ...) resolvable.
rosdep update --include-eol-distros

# The base image ships with an empty apt index; refresh it so rosdep can
# apt-install build dependencies (cv_bridge, pcl_ros, ...).
sudo apt-get update
